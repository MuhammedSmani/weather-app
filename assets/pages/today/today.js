...
function updateMainPageButtons(city) {
  const buttons = [
    {
      selector: ".todays__forecast_button",
      text: "Next Hours",
      href: "../../../assets/pages/hourly/hourly.html",
    },
    {
      selector: ".hourly__forecast_button",
      text: "Next 48 Hours",
      href: "../../../assets/pages/hourly/hourly.html",
    },
    {
...
function getTodaysForecast(data) {
  ...
  for (let period in timePeriods) {
    ...
    switch (period) {
      case "overnight":
        temp = Math.round(
          data.forecast.forecastday[1].hour[timePeriods[period]].temp_c
        );
        chanceOfRain =
          data.forecast.forecastday[1].hour[timePeriods[period]].chance_of_rain;
        var todayIconUrl =
          data.forecast.forecastday[1].hour[timePeriods[period]].condition.icon;
        var todayIconName = todayIconUrl.split("/").pop();
        todayIcon = getIconClass(todayIconName);
        break;
      default:
        temp = Math.round(
          data.forecast.forecastday[0].hour[timePeriods[period]].temp_c
        );
        chanceOfRain =
          data.forecast.forecastday[0].hour[timePeriods[period]].chance_of_rain;
        var todayIconUrl =
          data.forecast.forecastday[0].hour[timePeriods[period]].condition.icon;
        var todayIconName = todayIconUrl.split("/").pop();
        todayIcon = getIconClass(todayIconName);
        break;
    }
    todaysForecast.innerHTML += generateTodaysForecastHTML(
      period,
      temp,
      chanceOfRain,
      todayIcon
...
function generateHourlyForecastHTML(
  hourlyTime,
  hourlyTemp,
  hourlyRainChance,
  hourlyIcon
...
function generateDailyForecastHTML(
  shortDailyName,
  maxTemp,
  minTemp,
  dailyRainChance,
...
function getDailyForecast(data) {
  ...
  for (let i = 0; i < numOfDays && i < forecastdDaysData.length; i++) {
    ...
    if (i === 0) {
      ...
      shortDailyName =
        date.toLocaleString("en-US", { weekday: "short" }) + " " + day;
    }
    ...
    dailyForecast.innerHTML += generateDailyForecastHTML(
      shortDailyName,
      maxTemp,
      minTemp,
      dailyRainChance,
